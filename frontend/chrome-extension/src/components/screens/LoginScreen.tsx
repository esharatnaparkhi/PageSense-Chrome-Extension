...
export const LoginScreen = ({ onLogin, loading, error }: LoginScreenProps) => (
  ...
  >
    ...
    <motion.div
      ...
      className="mb-6 p-4 rounded-2xl gradient-brand shadow-glow"
    >
    ...
    >
      Your AI reading assistant for the web
    </motion.p>

    {error && (
      <motion.p
        className="text-xs text-red-500 mb-4 text-center bg-red-50 px-3 py-2 rounded-lg"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      >
        {error}
    ...
    >
      Summarize pages, ask questions, compare content
    </motion.p>
  </motion.div>
);
